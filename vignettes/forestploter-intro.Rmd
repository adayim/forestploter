...
```{r edit-plot, fig.width  = 7, fig.height = 7}
...
```{r multiple-group, fig.width  = 7.5, fig.height = 7.5}
dt <- read.csv(system.file("extdata", "example_data.csv", package = "forestploter"))
# indent the subgroup if there is a number in the placebo column
dt$Subgroup <- ifelse(is.na(dt$Placebo), 
                      dt$Subgroup,
...
# Saving plot
One can use the base method or use `ggsave` function to save plot. For the `ggsave` function, please don't ignore the `plot` parameter. The width and height should be tuned to get a desired plot.
