...
* The `edit_plot` function can be used to change the graphical parameter of text, background and CI. For example the color or font face of some columns or rows. 
...
The `add_text` function simply put the text in the plot without adding any rows to the plot. The `insert_text` function will insert a row and add text to the inserted row. This is the same as adding a blank row to the data before drawing a forest plot and use `add_text` to the row have. 

## Text justification and background
By default, all cells are left aligned. But it is possible to justify any cells in the forest plot by setting parameters in `forest_theme`. Set `core=list(fg_params=list(hjust=0, x=0))` to left align content, and `rowhead=list(fg_params=list(hjust=0.5, x=0.5)` to center header. Set `hjust=1` and `x=0.9` to right align text.   
