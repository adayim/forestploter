...
Same rule apply to change the background color by setting `core=list(bg_params=list(fill = c("#edf8e9", "#c7e9c0", "#a1d99b")))`. Change settings in `core` if you want to change graphical parameters for data, `colhead` for header. Change settings in `fg_params` to modify the text, see parameters for `textGrob()` in `grid` pacakge. Change `bg_params` to modify settings for background graphical parameters, see `gpar()` in `grid` pacakge. You should pass parameters as a list.

Provide a single value if you want cells have the same justification or vector for each cells. As you can notice that the second example justify text by row using the provided vector, and the vector will be recycled. More details can be found [here](https://cran.r-project.org/web/packages/gridExtra/vignettes/tableGrob.html).  
...
```{r multiple-param, out.width="70%", fig.width  = 10, fig.height = 8}
