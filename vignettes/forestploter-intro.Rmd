...
The data we have above will be uased as basic layout of the forest plot. The example below shows how to draw a simple forest plot. A footnote was added as a demonstration.
...
Now we will use the same data above, pretending that's what we have and add a summary point. We also want to change the graphical parameters for confidence interval and other parts of the plot. Theme of the forest plot can be adjusted with `forest_theme` function, check out the manual for more details.
...
* The `edit_plot` function can be used to change the graphical parameter of text and background. For example the color or font face of some columns or rows. 
...
## Text justification
By default, all cells are left aligned. But it is possible to justify any cells in the forest plot by setting parameters in `forest_theme`. Set `core=list(fg_params=list(hjust=0, x=0)` to set the contents of the table aligned to left, and set  `rowhead=list(fg_params=list(hjust=0.5, x=0.5)` to center header. Set `hjust=1` and `x=0.9` to right align text.  

Provide a single value if you want cells have the same justification or vector for each cells. As you can notice that the second example justify text by row using the provided vector, and the vector will be recycled. More details can be found [here](https://cran.r-project.org/web/packages/gridExtra/vignettes/tableGrob.html#text-justification).  

```{r text-justification, out.width="80%", fig.width  = 7, fig.height = 3}
dt <- dt[1:4, ]
...
## Arrow labels
You can let the arrow labels aling to the middle or the end of the arrow head. Many efforts have been made to make sure the arrow would be long enough to cover the whole x-asix if the label length is short or extend to the length of the label if the label is too long. But it is hard to compare the length of the label and x-axis, cause the viewport is determine after the arrow and label is created. The current solution is not perfect, please paste some leading or trailing space to get what you want.

# Multiple CI columns

Sometimes one may want to have multiple CI columns, each column may represent different outcome. If this is the case, one only need to provide a vector of the position of the columns to be drawn in the data. If the number of columns provided to draw the CI columns are same as number of `est`, one CI will be drawn into each CI columns. If the number of columns provided is less than number of `est`, the extra `est` will be considered as a group and will be drawn to the CI columns again. In the latter case, group number equals is calcualted as `length(est)/length(ci_column)` and multiple columns will be drawn into one cell. As seen in the example below, the CI will be drawn in the column 3 and 5. The first and second elements in `est`, `lower` and `upper` will be drawn in column 3 and column 5.

In a multiple groups example, two or more CI in one cell. The solution is simple, just provide all the values sequently to `est`, `lower` and `upper`. Which means, the first `n` elements in the `est`, `lower` and `upper` are considered as same group, same for next `n` elements. The `n` is determined by the length of `ci_column`. As it is shown in the example below, `est_gp1` and `est_gp2` will be drawn in column 3 and column 5 as normal, considered as **group 1**. The `est_gp3` and `est_gp4` will be drawn in column 3 and column 5 as normal, considered as **group 2**. 
